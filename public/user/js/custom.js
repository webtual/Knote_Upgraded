...
/**
Service section pre-questions
Next-Previous with auth
***********************/
$('.pre-questions').click(function(){
    var type = $(this).attr('data-type');
    var label = $(this).attr('data-label');
    $('#services').find('h2:first').text(label);
    $('.service-block').css({"display":"none"});
    $('#question-block-'+type).removeClass('d-none');
    $(".wizard-section:not(#question-block-"+type+")").remove();
    $('html, body').animate({
      scrollTop: $('#main').offset().top - 97
    }, 1000);

});

$('.next-btn').click(function(){
    var length = $('.qu-answer-block').length;
    var question_index = $('.qu-answer-block:not(.d-none)').attr('data-key');
    var last_question_index = (parseInt(question_index));
    var validation_status = process_validation_in_wizard_form();
    
    if(!validation_status){
        return false;
    }

    if(length == (last_question_index)){
        var url = $('.qu-answer-block:not(.d-none)').closest('form').attr('action');
        var redirect_url = $('.qu-answer-block:not(.d-none)').closest('form').attr('data-redirect-url');
        $.ajax ({
            type: 'POST',
            url: url,
            async: false,
            data: $('.qu-answer-block:not(.d-none)').closest('form').serialize(),
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            success : function(response) {
                if(response.status == 201){
                	$('#inquiry-thank-you').removeClass('d-none');
                	$('.pre-form').addClass('d-none');
                    //window.location.href = redirect_url;
                }else{
                    $('.validation').remove();
                    $('.form-group:last').append('<div class="validation">'+response.message+'</div>');
                    $('.btn-next-previous').css('bottom','35px');
                }
            },
            error: function (reject) {
                $('.validation').remove();
                if( reject.status === 422 ) {
                    var errors = $.parseJSON(reject.responseText);
                    var errors = errors['errors'];
                    $('input[name="'+Object.keys(errors)[0]+'"]').closest('div').append('<div class="validation">'+Object.values(errors)[0]+'</div>');
                    $('.btn-next-previous').css('bottom','35px');
                }
            }
        }); 
    ...
    if(length != last_question_index){
        $('.qu-answer-block:eq('+(question_index-1)+')').fadeOut().addClass('d-none');
        $('.qu-answer-block:eq('+(question_index)+')').fadeIn().removeClass('d-none');
    }

    if(question_index >= 1){
        $('.previous').removeClass('d-none');
        $('.btn-next-previous').css({"margin-right": "0px", "bottom": "15px"});
    }else{
        $('.previous').addClass('d-none');
        $('.btn-next-previous').css("margin-right", "90px");
    }
});
...
$('.previous-btn').click(function(){
    ...
    var question_index = $('.qu-answer-block:not(.d-none)').attr('data-key');
    $('.qu-answer-block:eq('+(question_index-1)+')').fadeOut().addClass('d-none');
    $('.qu-answer-block:eq('+(question_index-2)+')').fadeIn().removeClass('d-none');
    if(question_index <= 2){
        $('.previous').addClass('d-none');
        $('.btn-next-previous').css("margin-right", "90px");
    }else{
...
function process_validation_in_wizard_form(){
    
    $('.validation').empty();
    var rv = true; 
    $(".qu-answer-block:not(.d-none)").find("input, select").each(function() {
        if($(this).is("[required]")){
            if($(this).attr('name') != undefined){
                if(this.value == ""){
                    ...
                    $('.btn-next-previous').css('bottom','35px');
                    $(".qu-answer-block:not(.d-none)").find('.form-group').append('<div class="validation">The my preferences all field are mandatory.</div>');
...
$("#business_banner_image").change(function() {
  cropImage(this);
});

$('#business-proposals-add').click(function(){
    var url = $('#business-proposals-add').closest('form').attr('action');
    $.ajax ({
        type: 'POST',
        url: url,
        async: false,
        data: $('#business-proposals-add').closest('form').serialize(),
        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        success : function(response) {
            $('#business-proposals-add').closest('form')[0].reset();
            $(".summernote-editor").summernote("reset");
            $('.my-cu-check-uncheck-array').removeAttr('checked');
            toaserMessage(response.status, response.message);
        },
...
$('#business-proposals-update').click(function(){
    var url = $('#business-proposals-update').closest('form').attr('action');
    $.ajax ({
        type: 'PUT',
        url: url,
        async: false,
        data: $('#business-proposals-update').closest('form').serialize(),
        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        success : function(response) {
            toaserMessage(response.status, response.message);
        },
...
$('#update-preferences').click(function(){
    var url = $('#update-preferences').closest('form').attr('action');
    $.ajax ({
        type: 'POST',
        url: url,
        async: false,
        data: $('#update-preferences').closest('form').serialize(),
        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        success : function(response) {
            //toaserMessage(response.status, response.message);
             window.location.href = location.protocol + "//" + location.host+'/my-profile';
        },
        error: function (reject) {
            if( reject.status === 422 ) {
                var errors = $.parseJSON(reject.responseText);
