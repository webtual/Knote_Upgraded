              ...
              <h3>Document</h3>
               <div class="tab-content pt-0">
                  <div class="tab-pane active" id="settings">
                     <form action="{{ url()->current() }}" id="loan-application-four" name="loan-application-four" method="post" onsubmit="return false;">
                              ...
                              @foreach($application->get_documents_by_type($key) as $document)
                                <div class="col-3 fetch-documents">
                                  <div class="item">
                                      ...
                                      @endif
                                      <p class="hard-remove-document-image text-left font-20" data-id="{{ $document->id }}" data-url="{{ url('loan/create/document/delete') }}" data-application-id="{{ $document->application_id }}" >
                      ...
                      <div class="text-center">
                         <a href="{{ route('loan.create.finance') }}" class="btn btn-light waves-effect waves-light mt-2"><i class="fe-arrow-left"></i>&nbsp;
                            Previous
                            </a>
                         <button type="submit" data-url="{{ url()->current() }}" data-redirect="{{ url('loan/create/review') }}" id="pr-four" class="btn btn-success waves-effect waves-light mt-2 ml-2">Next<span class="btn-label-right"><i class="fe-arrow-right"></i></span></button>
                      </div>
                      @else
                      <div class="text-center">
                         <a href="{{ url('loan/edit/finance/'.$enc_id) }}" class="btn btn-light waves-effect waves-light mt-2"><i class="fe-arrow-left"></i>&nbsp;
                            Previous
                            </a>
                         <button type="submit" data-url="{{ url('loan/create/document') }}" data-redirect="{{ url('loan/edit/review/'.$enc_id) }}" id="pr-four" class="btn btn-success waves-effect waves-light mt-2 ml-2">Next<span class="btn-label-right"><i class="fe-arrow-right"></i></span></button>
                      </div>
                      @endif
