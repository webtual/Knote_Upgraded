...
<div class="content">
   ...
   <div class="container-fluid">
      ...
      <div class="row">
         <div class="col-lg-12">
            <form action="{{ route('my.profile.update') }}" id="personal-information" name="personal-information" method="post" onsubmit="return false;" enctype="multipart/form-data">
                @csrf
      ...
      @if( (!Gate::check('isAdmin')) && (!Gate::check('isAssistant')) && (!Gate::check('isLoanApplicant')) && (!Gate::check('isBroker')))
      <div class="row" id="my-preferences">
         <div class="col-lg-12">
            <form action="{{ url('my-preferences/update') }}" name="my-preferences" method="post" onsubmit="return false;">
               <div class="card-box d-none" id="my-preferences-text">
                  <h5 class="text-uppercase bg-light p-2 mt-0 mb-3">My Preferences</h5>
                  @if(!empty($pre_question))
                  @foreach($pre_question as $value)
                  @if($value->type == 'selection')
                  <div class="form-group mb-3">
                     <label for="">{{ $value->title }}<span class="text-danger">*</span></label>
                     <select class="custom-select" name="{{ $value->id }}" required="required">
                        <option value="">Select..</option>
                        @if(!empty($value->question_answers))
                        @foreach($value->question_answers as $op => $option)
                        <option value="{{ $option->answer_text }}" @if($value->user_answer != NULL){{($value->user_answer->question_answer == $option->answer_text) ? 'selected' : ''}}@endif >{{ $option->answer_text }}</option>
                        @endforeach
                        @endif
                     </select>
                  </div>
                  ...
                  <div class="form-group mb-3">
                     <label for="">{{ $value->title }}<span class="text-danger">*</span></label>
                     <input type="text" id="" name="{{ $value->id }}" value="@if($value->user_answer != NULL){{$value->user_answer->question_answer}}@endif" class="form-control" placeholder="{{ $value->title }}" required="required">
                  </div>
                  ...
                  <div class="form-group mb-3">
                     ...
                     <div class="type ml-1">
                        ...
                        <div class="radio radio-info form-check-inline">
                           <input type="radio" value="{{ $option->answer_text }}" name="{{ $value->id }}" @if($value->user_answer != NULL){{($value->user_answer->question_answer == $option->answer_text) ? 'checked="checked"' : ''}}@endif
                           >
                  ...
                  @endif
                  @endforeach
                  <button class="btn btn-primary mt-3" type="submit" id="update-preferences">Save</button>
                  <button type="reset" class="btn btn-secondary mt-3 ml-2">Cancel</button>
                  @endif
               </div>
               <div class="card-box" id="my-preferences-show">
                  <h5 class="text-uppercase bg-light p-2 mt-0 mb-3">My Preferences <a href="javascript:;" class=" font-15" id="preferences-info"><u>Edit</u></a></h5>
                  @if(!empty($pre_question))
                  @foreach($pre_question as $value)
                  <div class="form-group mb-3">
                     <label for="">{{ $value->title }}</label>
                     @if(!empty($value->question_answers))
                     @if($value->user_answer != NULL)
                     <p>{{$value->user_answer->question_answer}}</p>
                     @endif
                     @endif
                  </div>
                  @endforeach
      ...
      @endif
      <div class="row justify-content-center d-none" id="change-password">
         <div class="col-lg-12">
            <form action="{{ url('my-profile/change-password') }}" name="change-password" method="post" onsubmit="return false;">
               <div class="card-box">
                  ...
                  <div class="form-group mb-3">
                     ...
                     <input type="password" id="" name="old_password" class="form-control" placeholder="Old Password">
                  </div>
                  <div class="form-group mb-3">
                     <label for="">New Password<span class="text-danger">*</span></label>
                     <input type="password" id="" name="new_password" class="form-control" placeholder="New Password">
                  </div>
                  <div class="form-group mb-3">
                     <label for="">Confirm New Password<span class="text-danger">*</span></label>
                     <input type="password" id="" name="confirm_password" class="form-control" placeholder="Confirm New Password">
                  </div>

                  <button class="btn btn-primary mt-3" type="submit" id="btn-change-password">Change Password</button>
                  <button type="reset" class="btn btn-secondary mt-3 ml-2">Cancel</button>
               </div>
            </form>
         </div>
...
<script type="text/javascript">
   ...
   $('#preferences-info').click(function(){
      $('#my-preferences-text').removeClass('d-none');
      $('#my-preferences-show').addClass('d-none');
   });
</script>
@endsection