...
<div class="content">
   ...
   <div class="container-fluid">
      ...
      <div class="row">
         <div class="col-lg-12">
            <form action="{{ url('admin/email-templates/update') }}" name="admin/emailtemplates/edit" method="post" onsubmit="return false;" enctype="multipart/form-data">
               <div class="card-box" style="overflow-y: auto;height: calc(100vh - 225px);">
                    <div class="row">  
                        <div class="col-lg-6">
                            <div class="form-group mb-3">
                                <label for="">Title<span class="text-danger">*</span></label>
                                <input type="text" id="title" name="title" value="{{ $result->title }}" class="form-control" placeholder="Title">
                                <input type="hidden" id="temp_id" name="temp_id" value="{{ $result->id }}">
                                @error('title')
                        ...
                        <div class="col-lg-6">
                            <div class="form-group mb-3">
                                ...
                                <input type="text" id="subject" name="subject" value="{{ $result->subject }}" class="form-control" placeholder="Subject">
                                @error('subject')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="col-lg-12">
                            <div class="form-group mb-3">
                                ...
                                <textarea id="body" name="body" class="form-control summernote-editor" rows="4" placeholder="Body">{{ $result->body }}</textarea>
                                @error('body')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="col-lg-12 text-right">
                            <hr>
                            <button class="btn btn-success mt-3" type="submit" id="template-edit">Update</button>
                        </div>
                    </div>
                    <div class="row">  
...
<script>
   $('#template-edit').click(function(){
        var url = $('#template-edit').closest('form').attr('action');
        $.ajax ({
            type: 'POST',
            url: url,
            async: false,
            data: $('#template-edit').closest('form').serialize(),
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            success : function(response) {
                if(response.status == 200){
                    $('#template-edit').closest('form')[0].reset();
