            ...
            <div class="page-title-box">
               <div class="page-title-right">
                  <ol class="breadcrumb m-0">
                     <li class="breadcrumb-item active">Resource Inquiry</li>
                  </ol>
               </div>
               <h4 class="page-title">Resource Inquiry </h4>
            </div>
         </div>
      </div>
                  ...
                  <div class="mb-2">
                      <div class="row">
                          ...
                          <div class="col-2 text-right ">
                            <strong>Count : <span id="record-count"> {{ $inquiry->count() }}</span></strong>
                         </div>
                      </div>
                  </div>
                  ...
                  <div class="table-responsive">
                      <table id="demo-foo-filtering" class="table table-bordered toggle-circle mb-0" data-page-size="10">
                          ...
                          <tr>
                              <td>{{ $key + 1 }}</td>
                              <td>
                                @if($value->inquirieable_id != null)
                                  {{ App\Resource::whereid($value->inquirieable_id)->pluck('title')->first() }}
                                @else
                                -
                                @endif
                              </td>
                              ...
                              <td>
                                <form action="{{url('admin/inquiry/resources/'.$value->id)}}" class="cu-inline-block" method="POST">
                                  {{ method_field('DELETE') }}
                                  {{ csrf_field() }}
