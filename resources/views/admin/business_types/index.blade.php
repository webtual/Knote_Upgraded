                     ...
                     <div class="col-10 text-sm-center form-inline">
                        <div class="form-group mr-2" >
                           <select id="demo-foo-filter-status" class="custom-select custom-select-sm cu-foo-count">
                              ...
                              <option value="Business Proposals" data-count="{{ App\BusinessType::count_business_type(1) }}" {{ ( (Route::currentRouteName() == 'b.bt') || (Route::currentRouteName() == 'bt.create') ) ? 'selected' : '' }}>Business Proposals</option>
                              <option value="Resources" data-count="{{ App\BusinessType::count_business_type(2) }}" {{ ( (Route::currentRouteName() == 'r.bt') || (Route::currentRouteName() == 'r.create') ) ? 'selected' : '' }}>Resources</option>
                           </select>
                        </div>
                  ...
                  <table id="demo-foo-filtering" class="table table-bordered toggle-circle mb-0" data-page-size="10">
                     ...
                     <tbody>
                        ...
                        <tr>
                           ...
                           <td>{{ ($value->type == 1) ? 'Business Proposals' : 'Resources'  }}</td>
                           <td>{{ display_date_format($value->created_at) }}</td>
                           <td>
                              <form action="{{url('admin/business-type/'.$value->id)}}" class="cu-inline-block" method="POST">
                                 {{ method_field('DELETE') }}
                                 {{ csrf_field() }}
                                
                                 <a href="javascript:;" class="action-icon edit-business-types" data-id="{{ $value->id }}" data-name="{{ $value->business_type }}" data-type="{{ $value->type }}" title="Edit"> <i class="fe-edit"></i></a>
...
<div id="custom-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" style="display: none;" aria-hidden="true">
    <div class="modal-dialog">
      <form action="{{ url('admin/business-type/create') }}" method="post" role="form" class="business-types-form" onsubmit="return false;" data-redirect-url="{{ ( (Route::currentRouteName() == 'bt.create') || (Route::currentRouteName() == 'b.bt') ) ? url('admin/business-proposals/categories') : url('admin/resources/categories') }}" >
      @csrf
           <div class="modal-content">
               ...
               <div class="modal-footer">
                  ...
                  <button type="submit" class="btn btn-primary waves-effect waves-light" id="submit-business-type">Submit</button>
               </div>
           </div><!-- /.modal-content -->
      </form>
    </div><!-- /.modal-dialog -->
